...
def get_product_by_model(model: str) -> Optional[Dict[str, Any]]:
    """根据型号查询产品"""
    results = execute_query(
        "SELECT * FROM products WHERE model ILIKE %s", (f"%{model}%",)
    )
    if not results:
        return None
    r = results[0]
    return {
        ...
        "refresh_rate": r.get("refresh_rate"),
        "srgb": r.get("srgb"),
        "dci_p3": r.get("dci_p3"),
        "delta_e": r.get("delta_e"),
        "ram_max": r.get("ram_max"),
        "ssd_slot": r.get("ssd_slot"),
        "battery_wh": r.get("battery_wh"),
        "weight": r.get("weight"),
        "thickness": r.get("thickness"),
        "has_direct_gpu": r.get("has_direct_gpu"),
        "has_water_cooling": r.get("has_water_cooling"),
        "has_rgb": r.get("has_rgb"),
...
def get_all_products() -> List[Dict[str, Any]]:
    """获取所有产品"""
    results = execute_query("SELECT * FROM products ORDER BY official_price DESC")
    return [dict(r) for r in results]


def get_products_by_series(series: str) -> List[Dict[str, Any]]:
    """根据系列查询产品"""
    results = execute_query("SELECT * FROM products WHERE series = %s", (series,))
    return [dict(r) for r in results]
